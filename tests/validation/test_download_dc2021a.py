...
def test_download_reproduces_structure_and_contents(tmp_path):
    ...
    download. Re-downloads ~1 GB; marked external (on-demand). Skips (not fails) when
    offline — BOTH halves: the probe below, and the transfer inside `network_guard`,
    because a mirror that answers the HEAD and then stalls partway through ~1 GB is the
    same offline condition (observed 2026-09-14, httpx.ReadTimeout). The SHA256 asserts
    stay OUTSIDE the guard: byte-exactness is the subject of this test and must never be
    converted into a skip.
    """
    skip_if_unreachable(dl.MEOM_BASE)
    with network_guard(dl.MEOM_BASE):
