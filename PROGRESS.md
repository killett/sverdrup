...
> is not (261).
>
> The spec's Gate-1 criterion was READ AT CLOSURE (262): phase14.locked_tally ABSENT;
> c2 tally {"miost5": 3, "miost6": 1}; legacy list unchanged (mirror check PASS). ⛔ The
> per-run tally guard is NOT that evidence: it watches phase14.locked_n, and the ceremony
...
>
> ## 📌 WHAT STAGE 2 INHERITS — one place, not this block
>
> **`docs/superpowers/2026-09-21-phase14-gate1-closure.md`** carries all of it: the pin-260
> accounting, pins 261/262 verbatim, the four closure reads with their trip conditions, the
> C1→2 contract AT CLOSURE with each line citing its witnessed node, the twelve named
> Stage-2 obligations (263), and six FOUND-BY-SWEEP items recorded but not acted on.
> **Migrate, don't duplicate** — the obligations are NOT restated here, so there is one
