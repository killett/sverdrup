...
- **OSSE:** TRUTH provider interface re-armed (dormant since 4b). Marginal value = truth for tiles without a signed reference (the SO tile's kernel decision, constraint 3; covariate-transfer checks) — the seam ORACLE already covers seams in the anchor footprint. Run decision RULED at Stage 1 (pins 250/251): WAIT — validity is prior to price. Admissible truth is a free-running nature run (LLC4320-class), never a reanalysis that assimilates the constellations under test (GLORYS12 disqualified). Common-span design and replication are Stage 2's.
...
11. Every quantitative gate names, at design time, the measurement condition under which it could fail. A gate discharged by a measurement that could not have tripped it is not discharged — it is unrun. Record the condition beside the threshold. *(Ruling pin 24, 2026-07-25. The list below is the argument for why the rule is mechanical rather than a matter of judgement (pin 100d, 2026-08-29). **The count is DERIVED from the list, never restated beside it** (owner pin 146b, 2026-09-02): a stated count that drifts from its own enumeration is a small instance of the family this list exists to catalogue, and it had drifted — the entry said "six" while enumerating seven. Each instance carries a sequential tag and `tests/test_project_context_instances.py` asserts the tags are contiguous, unique and complete, so the count cannot drift again.* **(i1)** the Stage-0 T11 vacuous pin; **(i2)** the T0 source-scan pin; **(i3)** the T2 1.3× bracket run against iteration-capped solves; **(i4)** pin 32's 3×; **(i5)** the ±4 sd acceptance; **(i6)** the s\*/χ² identity — two fields that are the same expression `mean((truth-mean)**2/var)` on the same points, so their agreement could never have failed to hold and must never be read as corroboration (pin 100); **(i7)** the bare `if not track.exists()` reuse check (pin 110b, 2026-08-30) — an existence test cannot distinguish a legitimately built artifact from a leftover, so it silently adopts whatever is on disk, which is how a test-written file came to sit in the evidence directory wearing a `STAGE1-EVIDENCE` label; reuse is now gated on a build record: recorded sha, tile and mission, or a loud refusal; **(i8)** — owner-authored, pin 140(b), 2026-09-02 — **the rule's own ENFORCEMENT.** `validate_gate_schema` inspects a block only if the block declares `kind: "gate"` or `kind: "validation"` about itself. Swept over the live store, `kind: gate` appears ZERO times and `kind: validation` exactly once, so pin 42's reachability refusal has never fired on anything and pin 78's range rule has only ever inspected the instance that produced it. The mechanism, in the owner's words: **A SCHEMA FIELD THAT ONLY INSPECTS VOLUNTEERS INSPECTS NOTHING.** It belongs in this list precisely because the owner wrote the rule it breaks. The correction is to key on SHAPE — `projection_audit` (pins 134/139, refusing, with the eleven caught blocks declared by forward-pointer amendment) and `verdict_audit` (pin 140a, reporting first per 140c) **(i9)** — owner-authored, pin 210(a), 2026-09-18 — **the ±66 attestation in the Gate-1 pack.** It attested that every diverse tile's SOLVE BBOX lies inside ±66; pins 10/16 define the breach on the OBS EDGE (`solve_bbox.lat_min − halo`). A solve bbox is always inside its own obs frame, **so the quantity cited could not breach** — true and unfailable, in an attestation, in the pack that carries this very list. Restated on the poleward obs edge with southern's **1.0°** margin named, derived rather than typed, and test-pinned against the store.)*
12. A check earns its placement by where the error it catches **originates**, not by where the value is consumed. An invariant guarding a *construction* error belongs where construction happens — it is present from the first line and costs seconds to find there, and a leg to find at the end. *(Ruling pin 103d, 2026-08-30; third costume of pin 83 — formatter before the suite — and pin 89 — measure before the 5-day decision. Instance: the s\*/χ² invariant, whose row-build raise KEEPS its place (103b) but now fires first on synthetic inputs in `preflight_scores_construction`.)*
13. **A narrower check may not override a broader one that has already spoken.** Once a gate reports a failure, only a rerun of *that* gate at *that* scope clears it — a narrower or different check is evidence about something else. And where a check emits a warning beside its verdict, the warning is **part of** the verdict: a test whose green line can be read without its warnings is mis-designed, so make the warning fail the exit code. *(Ruling pin 129, 2026-08-31. Two instances, both where the authoritative signal came FIRST and a weaker one was run afterward and believed: the commit hook's `mypy .` reported Failed over 419 files and was answered with `Success` over 1; R5 attempt 1's `WARNING: still alive` was correct and was read past to a `BIT-IDENTICAL` two lines below — from a run that was never killed. Distinct from discipline 11's family: these checks **worked**.)*
14. **Formatters run BEFORE any suite whose result will support a commit** — not only inside the mechanised gate. Prefer running `scripts/phase14_gate_suite.py` outright for any commit whose evidence will be cited: a second ordering discipline maintained by memory drifts from the mechanised one. *(Ruling pin 131, 2026-09-01. Pin 83 fixed this ordering where it was mechanised and left it broken everywhere else, so the defect recurred structurally on ordinary commits — a 1552-passed suite described the pre-format tree, and the hook then reformatted a blank line, an import merge and two line wraps. Disclosed and re-run per 83(b) rather than papered over.)*
15. **A dual review may be substituted by owner ratification only where the defects that mattered were caught by VERIFICATION AGAINST AN ARTIFACT — and the substitution is stated, never claimed as the stronger form.** *(Ruling pin 212, 2026-09-18.* **Accepted instance:** Stage 1's four production legs. No two-reviewer document exists for them; what exists is the owner's per-leg ratification, and every defect that mattered was caught by an artifact check rather than by review — `h2ag` by R5's dry run, the pin-151(b) headroom drop by reading the row back out of the store, the attrs bug and the unrecorded band cut by sweeps. A retroactive review document would have produced paperwork, not scrutiny. *The Gate-1 pack §2.3 states the substitution plainly, which is the condition of accepting it.* **⛔ THE LIMIT — this is NOT the standard.** It does **not** extend to **DECISION packs**: T6, T7 and T8 each require the full two-reviewer form (pin 39 — two reviewers, named attack surfaces authored by the requester, verdict CONFIRMED / OVERTURNED / UNDER-EVIDENCED), because a decision pack is reasoning rather than measurement, which is where review earned its keep at T13 and where no artifact check can stand in for it.)*
16. **A consequence is priced at the EXTREME that governs it, never at the centre of its own distribution.** A quantity that varies across a domain must be evaluated where it binds — the poleward reach, the worst case, the edge the constraint actually tests — and a figure quoted at the mean or the midpoint understates it while looking measured. *(Ruling pin 219(e), 2026-09-19; DISTINCT from discipline 11's unfailable-check family — this check can fail, it is simply evaluated in the wrong place.* **Instances, both from T6 in one task:** **(p1)** the draft kernel pack priced its ±66 BREACH on a multiplier evaluated at the tile's φ0; **(p2)** the corrected pack's km-space halo row did the same thing again — 1/cos φ at φ0 = −54.4° gives 1.7179° and clears, while the latitudes the tile actually SOLVES give 2.1301° at the core edge and 2.2812° at the solve-bbox edge, both of which **breach**. (p2) survived a two-reviewer adversarial review and was caught only when the owner asked which direction the consequence ran.)*

17. **Under pin 212(b), the SECOND reviewer attacks the FRAME, not the list.** Reviewer A works the requester's named attack surfaces; reviewer B is briefed independently to attack the **frame** — the cost model, the unit of account, the assumption every named surface sits on top of. **The requester does NOT author B's targets, because the requester is the one who chose the frame.** *(Ruling pin 241, 2026-09-20. DISTINCT from discipline 15: that one says owner ratification cannot substitute for dual review; this one says a well-authored surface list is itself a blind spot, and it is the **OWNER's** blind spot, not the executor's. The evidence FOR the discipline is that the surfaces were good — pin 235's five were all CONFIRMED by reviewer A on substance, and the defect still sat underneath them.* **(f1)** T8's OSSE pricing: the five authored surfaces asked whether every figure traced to a CONVERGED measurement, whether the truth cost was omitted, whether the value-case string overstated, whether the tile was named, and whether the empty decision cell read as vacuous. **All five were confirmed.** The document priced *one epoch class = one tile solve*, treating solve wall as independent of observation count — and the four legs (identical bbox, m, window plan and constellation, with only n_obs varying) give **wall ∝ n_obs^1.4146, R² 0.9989**, while the classes run 3–9 missions against the legs' 5. **It mispriced the subset table the owner would have chosen a scope from.** ⚠ **CORRECTED 2026-09-20 under pin 244(b): the replacement claim — that post-lift is dearer 'at every exponent above zero' — IS ITSELF WITHDRAWN AND FALSE.** Cost is Σ f(mᵢ); post-lift has FEWER and LARGER classes, so the ordering needs f CONVEX, not monotone, and it reverses below p ≈ 0.638. See **(u3)** in discipline 18. *A checklist gets a checklist's answer, however rigorous.* **(f2)** the same document's editorial — *"cost and evidential value move together"* — was an artefact of the same flat model and **inverted with it**: the sparse historical constellations, where the era-transfer claim is weakest, carry the fewest observations and are the CHEAPEST classes. A frame defect propagates into the prose the frame generated, which no surface-level check reads as a claim at all. **(f3)** the pinned corroboration was already in the tree and unconsulted: `tier2_probe_kuroshio_m100` recorded `implied_exponent 1.28` with the verdict *"NOT linear … optimistic by 1.42×"*, so the flat model was **flatter than linear** — a model this project had already measured and rejected once. **A frame is the thing you do not think to look up.**)*

18. **A price is stated in a UNIT, and the unit must be the thing that VARIES.** Before quoting a cost, name what changes between the things being priced and check that the unit of account is that quantity — not a proxy for it, and not an aggregate over it. *(Ruling pin 248, 2026-09-20. Three instances in one task, each caught only by review, and the third is the owner's own.* **(u1)** T8 v1 priced *one epoch class = one tile solve*. What varies between epoch classes is the CONSTELLATION, and solve wall moves with observation count, so the unit (classes) was not the varying quantity. Caught by the frame reviewer; the surfaces reviewer confirmed all five authored surfaces on the same document. **(u2)** T8 v2 fitted `wall ∝ n_obs^1.41` and then applied that exponent to MISSION COUNTS — measured on one axis, applied on another, with a pin-139 declaration whose `measured_over` was in observations and whose `application_range` was in missions, so it bounded nothing. The legs refute the proxy directly: **n_obs varies 1.26× at a FIXED five-mission constellation**, because observation count is a function of (constellation, tile), not of constellation alone. **(u3)** — owner-authored, pin 244 — the ratification that ordered the v2 rebuild computed `(Σm_post / Σm_pre)^p`, applying the exponent to the AGGREGATE rather than summing per-class costs. **Σ(mᵢ^p) ≠ (Σmᵢ)^p unless p = 1.** The check ran at p = 0.5, 1.0, 1.4146 and 2.0; **p = 1 is the only point at which the two formulas can agree, and it was the only one that did** — the agreeing figure was read as verification of the other three. ⭐ **The lesson is not 'check the arithmetic'. It is that a single agreeing figure is not corroboration when it is the one point where disagreement is impossible** — the same shape as discipline 11's unfailable checks, arriving in a VERIFICATION rather than in a gate.)*

19. **When a deliverable is overturned TWICE on the same frame, the next ruling examines the DELIVERABLE, not the fix.** The question *"should this be produced at all?"* becomes live at the second overturn, and ordering a third rebuild is a decision to keep answering a question nobody has established is worth answering. *(Ruling pin 253, 2026-09-20 — **owner-authored, and the instance is the owner's own**.* **(d1)** T8's OSSE pricing. Three rebuilds were ordered, each overturned on the UNIT OF ACCOUNT (§7 discipline 18, (u1)-(u3)), and **each rebuild MOVED the defect rather than removing it**: v1 priced in classes; v2 fitted on observations and applied to missions; v3 removed the exponent from the headline and **left it in the only section that produced a verdict**. The fourth review then found what had been available since the first: **the experiment may not answer its own value case** — no replication, a truth field that ASSIMILATES the constellations under test, and an epoch-span question preserving "FIXED truth" under NEITHER reading. ⭐ **VALIDITY IS PRIOR TO PRICE**, and the price of an experiment that may establish nothing is not a useful number. T8 closed as a ruled WAIT at pin 250. *The owner's own words: "I ordered three rebuilds of a deliverable whose frame was wrong ... The question 'should this be priced at all?' was available at the first overturn and I did not ask it."*)*
...
- This document lives at `docs/project-context.md` and is **read FROM THE
  ...
  clone → **`sh scripts/install_git_hooks.sh`, then `sh scripts/resume_checks.sh`**
  → read `docs/project-context.md` → read the PROGRESS.md banner
...
- **Why the hook step is FIRST and not a gotcha (ruling pin 104, 2026-08-30):**
  `.git/hooks` is not versioned, so every clone and every box rebuild starts
  with **no post-commit hook**, and nothing says so — origin sat at `ba35d80`
  for a whole stage while every report line claimed the tree was pushed.
  There was never a hook that failed; **absence looked like success.**
  `scripts/resume_checks.sh` therefore REPORTS hook presence, any
  `.git/UNPUSHED_COMMITS` marker, local-vs-**remote** head (via `ls-remote`,
  pin 102b — never `rev-parse` alone) and tree cleanliness, and **exits
  non-zero** when any of them is wrong.
