...
11. Every quantitative gate names, at design time, the measurement condition under which it could fail. A gate discharged by a measurement that could not have tripped it is not discharged — it is unrun. Record the condition beside the threshold. *(Ruling pin 24, 2026-07-25. **Six instances to date** — the list is the argument for why the rule is mechanical rather than a matter of judgement (pin 100d, 2026-08-29): the Stage-0 T11 vacuous pin; the T0 source-scan pin; the T2 1.3× bracket run against iteration-capped solves; pin 32's 3×; the ±4 sd acceptance; the s\*/χ² identity — two fields that are the same expression `mean((truth-mean)**2/var)` on the same points, so their agreement could never have failed to hold and must never be read as corroboration (pin 100); and **seventh, the bare `if not track.exists()` reuse check** (pin 110b, 2026-08-30) — an existence test cannot distinguish a legitimately built artifact from a leftover, so it silently adopts whatever is on disk, which is how a test-written file came to sit in the evidence directory wearing a `STAGE1-EVIDENCE` label. Reuse is now gated on a build record: recorded sha, tile and mission, or a loud refusal. **And — owner-authored, pin 140(b), 2026-09-02 — the rule's own ENFORCEMENT joined the list: `validate_gate_schema` inspects a block only if the block declares `kind: "gate"` or `kind: "validation"` about itself. Swept over the live store, `kind: gate` appears ZERO times and `kind: validation` exactly once, so pin 42's reachability refusal has never fired on anything and pin 78's range rule has only ever inspected the instance that produced it. The mechanism, in the owner's words: A SCHEMA FIELD THAT ONLY INSPECTS VOLUNTEERS INSPECTS NOTHING. It belongs in this list precisely because the owner wrote the rule it breaks. The correction is to key on SHAPE — `projection_audit` (pins 134/139, now refusing, with the eleven caught blocks declared by forward-pointer amendment) and the pin-42 re-key (pin 140a, sweeping and reporting first: 36 verdict- or threshold-bearing blocks, none declaring pin-42 fields).** *(Bookkeeping note, recorded rather than silently fixed: this parenthetical says "Six instances to date" and then enumerates more than six. The count has drifted from the list; the list is the argument, and it is the list that is authoritative.)*)*
