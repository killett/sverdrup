...
**Files:**
- Modify: `scripts/phase14_stage1_run.py` — **⚖ CORRECTED (owner pin 92 ratified the finding).** The original line read *"only if gaps surface; the `run` command from Task 1 should already do these"*. **It does not:** `_solve_leg` raises `NotImplementedError` and `run` refuses at the stub. **T5 is a substantial BUILD before any leg runs** — the real load/solve/score/record leg, built against Task 4's `_seam_pair_real_leg` as the working analog.
...
**Acceptance Criteria:**
- [ ] Four runs recorded under `phase14.stage1.tiles.{equatorial,southern,quiet_gyre,kuroshio}` — each: µ/λx/coverage/χ² j3-validation rows + raw-σ + LABELED scalar-s* reference row + bridge_caveat verbatim + seal sha. **⚖ OWNER PIN 94 — the raw-σ row carries `SIGMA_CAVEAT` as a REQUIRED SCHEMA FIELD**, attached at `build_evidence_row` exactly as `BRIDGE_CAVEAT` is (a row the σ side cannot be built without, never prose at write time), citing `phase14.stage1.crn_production_defect_deferred` in pin 87's terms — *a property of the SHIPPED SYSTEM, not of an instrument*. **Scope it honestly (94f):** these are per-tile σ levels under per-tile CRN origins; **cross-tile σ comparison is NOT supported**; the boundary gradient is deferred and named — and it must **NOT** imply the within-tile level is compromised (**it is not**: the four diverse tiles are pairwise disjoint; only `seam_n`/`seam_s` are adjacent, pin 68). **The row is NOT dropped** (94d): fork-d pin 6 pre-registered it and Stage 2G needs these levels for pin 86(a)'s inheritance package. **⚖ OWNER PIN 123 — THE h2ag→h2g RELABEL IS AN INTERPRETATION AND TRAVELS WITH THE BRIDGE CAVEAT, in the same breath, not a separate note.** The anchor uses `h2g` natively; the diverse tiles use CMEMS `h2ag` RELABELLED to `h2g`, so the two sides differ in a component of the frozen config. The golden-tile bridge delta was measured **with the relabel in place**, so it already absorbs whatever the relabel costs — **AT THE ANCHOR BOX**; per review pin 7 that does not transfer per-tile, so **the relabel's cost at the diverse tiles is UNMEASURED, with exactly the structure of the bridge caveat itself** (123a). The `mission_relabel` row field is the mechanism; the pack must say what it MEANS (123b). **Alternatives refused, recorded (123c):** dropping `h2ag` changes the constellation and breaks like-for-like; deriving an R for `h2ag` unfreezes the config. **Relabelling is the least-bad option, not a free one.** **⚖ OWNER PINS 95+98 — pin-42 fields go on the χ² j3-validation row ONLY**; everything else is report-only and TEST-PINNED as such. The χ² field **RECORDS, it does not GATE**: `harness.py:1145` made χ² deliberately non-gating (*"coverage remains the only bar"*) and pin 42 does not reverse that — **the non-gating status is itself test-pinned so a later reader cannot "complete" the bar that was deliberately left out** (98b)
- [x] **GroundTrack wiring — LANDED T5c (wiring only; see the standing-row note below):** every tile×era has its applicability evaluated through the EXISTING `Registry.applicable` + `report_rows` machinery (Phase-11; GroundTrack is the reference-free family's founding member, the 0.410→0.331 lineage); each tile×era yields either a standing row or a RECORDED ABSENCE — absence means absence and is recorded as such (fork F). NO new GroundTrack producers; test pins the wiring path, not new surfaces.
  **T5c note (2026-08-30, owner decision owed):** the wiring is in
  (`build_tile_report_block` → `report_only_instruments_block` →
  `Registry.applicable` + `build_report_rows`, recorded at
  `phase14.stage1.report_rows.<tile>.<era>`, both branches test-pinned). **In
  production the four diverse tiles will take the RECORDED-ABSENCE branch for
  GroundTrack**: the only orbit-geometry artifact in existence is derived over the
  CHALLENGE box (`build_geometry_artifact` fixes `_LON_LO/_LON_HI` and φ0=38.1) from
  the dc_obs L3 files, and using Gulf-Stream geometry at a Pacific tile would be
  geometry from the wrong place. Deriving per-tile geometry from the CMEMS dailies
  is a NEW producer, which this criterion forbids — so it is NOT done here and is
  surfaced instead
- [x] **Interpretation withholding is STRUCTURAL (review pin 8) — LANDED T5b:** `build_evidence_row` output keys test-pinned as EXACTLY the schema set — no free-prose field exists to interpret in; THIS is the control (review pin 17). The four-word row-serialization blocklist ("suggests"/"consistent with"/"attributable"/"implies") stays as a decorative tripwire, not the control; the pack-level absence check lives in T9 where the free prose actually lives
- [x] **Equatorial persistence (1-6) — LANDED T5d:** beyond the pack row, persist maps + evidence pack + FROZEN fold/eval frame to `phase14_stage1/equatorial_lane0/` — the future wave-increment comparison substrate; a `lane0_manifest.json` with per-file shas; recorded under the frozen-config policy sentence (fork-b pin 2 verbatim in the manifest)
- [x] **Southern Ocean — LANDED T5d (see the absence note):** additionally records measured anisotropy inputs for Task 6 (per-direction spectral/track diagnostics the kernel pack consumes — reuse `GroundTrack`/`SpectralFidelity` instrument families from T11 configs, per-tile×era parameterization).
  **⚖ T5d ABSENCE NOTE (2026-08-30) — the PER-DIRECTION half cannot be measured here, and the record says so.** What IS recorded at `phase14.stage1.anisotropy_inputs.southern.<era>`: the **grid anisotropy COMPUTED from the tile's own axes** (`PlaneGrid.from_deg`; `dy/dx ≈ 1/cos(φ₀) ≈ 1.72` at ~54.5°S — the quantity the kernel decision turns on) and the **SpectralFidelity row CITED from the recorded report block**, never recomputed (one producer per number). What is NOT: the **per-direction track diagnostics**, which come from `groundtrack`'s ORBIT_GEOMETRY — challenge-box scoped per pin 106, so `Registry.applicable` correctly excludes it here. Recorded as an ABSENCE naming the missing context and the ruling, with the consequence stated for T6: **its anisotropy evidence is the grid geometry plus the ISOTROPIC ring spectrum, and T6 must not present its arithmetic as though per-direction sampling had been measured**
- [x] **Kuroshio — LANDED T5d:** land-mask path assertions — dropped-land handling in framing/scoring exercised; `n_scored_points` honest; any all-land core refusal surfaced not swallowed
- [x] No interpretation prose anywhere — numbers + caveat only (owner rider; enforced by the structural pin above, not by intent). **T5e: the row-serialization tripwire fires on the tile-row path and the pinned caveats themselves record clean; the pack-level check remains T9's**
- [ ] Each run detached + stall-watched; RAM predicate before each; zero touches throughout
- [x] **Pin-12 gate breadth — ⚖ DISCHARGED BY RULING (owner pin 90), never "met", never "dropped".** The criterion was written while the box election was open. It closed 2026-07-25 (box KEPT, −4…11°N); **no `box_election_pending` state exists and NONE IS TO BE CREATED — pin 42 bars a gate that cannot fire.** Recorded at `CRITERION_8_DISCHARGE` with two load-bearing test citations (`test_run_equatorial_reaches_gated_stub_after_pin12_ruling` proves the gate is gone; `test_equatorial_frame_is_the_pin12_ruled_box` proves the RULED box survived — different facts, both load-bearing). **The REFUSAL half is dead. The BREADTH half is LIVE (pin 90c):** extend the test to the programmatic path (`record_evidence_row` for "equatorial") when `_solve_leg` lands
...
- [ ] **±66 breach column (review pins 10+16 — derives from the RULED frame, never a typed constant):** per option, the operative halo AND the resulting SO obs southern edge, computed as `edge = southern_frame.solve_bbox.lat_min − halo` READ OFF the registry frame; the breach threshold is COMPUTED (`halo_breach = 66 + solve_bbox.lat_min`), not typed. **⚖ OWNER PIN 99(a) — THE BRANCH IS COLLAPSED:** pin 2 was ruled **production-representative** 2026-07-25, so **only `halo ≤ 2.0` is live** (edge = −(64 + halo)); the "isolated" branch (edge = −(62 + halo), breach at halo > 4.0) is **dead arithmetic and is removed**. **Keep the derivation from the frame per pin 16** so the arithmetic stays COMPUTED, not typed — collapsing the branch must not turn the threshold into a constant. Any breaching option FLAGGED "±66 BREACH — owner ruling required"
...
- Budget: **⚖ OWNER PIN 99(b) — REWRITTEN AGAINST THE LIVE CEILING.** The original clause read "Tier 0/1 only; if any lane's sizing demands Tier 2 → that lane WAITS (pre-registered-or-WAIT; **no new ceilings exist**)". That premise became **false** when task 22 cleared the Tier-2 crossing and E-16 set an explicit ceiling. **A lane affordable under the 40 h per-leg ceiling is NOT a WAIT.** A lane is a WAIT only if it exceeds the live ceiling (per-leg ~40 h wall; launch gate `MemAvailable ≥ 2 × the MEASURED 4365 MiB`), and the sizing numbers are recorded either way.
...
**Acceptance Criteria:**
- [ ] Price from measured constants: one OSSE = (truth-provider generation, existing dormant interface) + (constellation-varied re-solves ≈ N_epoch-classes × tile solve wall) — a table in compute-hours, no cloud. **⚖ OWNER PIN 99(c) — THE PRICING BASIS IS CORRECTED: price from the CONVERGED numbers, never the CAPPED T2 probe.** Pin 23(a) ruled the original Task-2 probe ran BOTH legs into the 500 cap over rtol, so it is **unusable for absolute claims**. Use **pin 89's probe (3.440 h/window, 4365 MiB peak, 441/486 iterations, CONVERGED)** plus the anchor gate, and **T5's own leg actuals once they exist**. **State the basis in-row** so a reader can see which numbers priced it. Tier framing follows the live ceiling (task 22 + E-16), not "Tier 1 only"
...
**Acceptance Criteria:**
- [ ] Pack contains, owner-items first: (1) **the anchor gate's RULED ACCOUNTING — never "five green"** (owner pin 97b): TWO checks run and passed (1, 5), TWO cited and pre-ratified at Gate 0 (2, 4), ONE proxy-passed with the specified era no-op DEFERRED; cite `scripts/phase14_anchor_gate.py:164-177` as the discharge, whose own words are *"'five green' does not"* survive careful reading in Stage 2 (with cross-host slot pending-T18 explicit — and T18 is now a **STAGE 2** task per pin 86c); (2) **the seam result in its ruled shape** (owner pin 97c): **TWO attributable mean CLEAN verdicts and TWO σ cells NOT_ESTABLISHED**, stating plainly that **Stage 1 has NO attributable σ-route seam verdict** — per pin 37(b) the σ seam question is UNANSWERED, not answered clean; **pin 37(b)'s firewall holds: the diagnosis-derived bound must NOT sit adjacent to the UNMEASURED rows in any form a reader could take as a verdict**; + ORACLE numbers (with the 10×5 non-production-geometry sentence); (3) **FOUR transfer readings** (numbers + bridge caveats) — **one per diverse tile** (owner pin 97a: the store settles it — `seam_n`/`seam_s` carry no `scores`, no `reference_row`, no `bridge_caveat`, they are solve records not readings, and the artifact's own `non_transfer_note` says so; the anchor is the identity subject, not a transfer reading). **Test-pin the count against the store so it cannot drift back**; (4) kernel decision pack (decision cell empty); (5) revisit verdict rows; (6) OSSE priced section (decision cell empty); (7) **six-mission-refresh election presentation** — presumptive rule verbatim: instrument-class match, δ_j3 := δ_j2n (Poseidon-series); own chain + touch if elected; scope = Stage-2G assembly onward; (8) spend actuals vs Tier-0/1 ($0 expected; any WAIT rows); (9) discipline attestation — **⚖ OWNER PIN 124(a), ADOPTED VERBATIM: "zero locked opens, one deferred production defect named at `crn_production_defect_deferred`"** (as previously phrased it read as a clean bill, and pin 87 makes it not one), tally byte-identical, ±66° respected under the ruled convention, seal `check` PASS; (10) the T11 sealed-instrument coverage table **and, beside it, the T12 C1→2 contract coverage table** (owner pin 137 — T12's own AC requires it posted beside the instrument table, and this list is the slot); **(11) pin 61 — 58(d)'s RESULT** (three of four check-1 routes would have re-passed against a substituted reference; `gamma_route` recorded neither path nor sha; both fixes, the capture caveat and its reconciliation **including the searched-and-absent one**) — *"a reader who learns this from a remediation commit learns it in the wrong order"*; **(12) pin 86 — the σ question OPEN with the FULL inheritance package enumerated** (mechanism, both channels quantified, reachability + m requirement, pin 31(b)'s latitude non-uniformity, the ρ model with its validated span declared) and pin 37(c) as a CONTRACT LINE; **(13) pin 87 — the CRN PRODUCTION DEFECT in its own words**: a property of the SHIPPED SYSTEM, not of an instrument, and **Stage 2G CANNOT CLOSE while it stands**. **Pack vocabulary (review pin 21): "Rule 0" names ONLY the floor-probe attributability rule; the T0 residual check is always "the solve-validity guard"**
- [ ] **⚖ OWNER PIN 119 — THE 0.331 REPRODUCTION IS LABELLED A DETERMINISM CHECK, NOT CONFIRMATION.** The post-hoc recovery (pin 114) reproduced the anchor's `track_excess_log10_max_repeat` = `0.331012884019381`, bit-identical to the Phase-13 Gate-1 pack's winner figure. Wherever the pack states it, it states WHICH KIND it is: the anchor maps were already bit-identical to the phase-13 winner (check 1, max|Δ| = 0), so an identical score **follows from identical inputs**. It is an end-to-end WIRING test that could have failed (wrong map, wrong mission filter, wrong geometry artifact, broken lookup); it is **NOT independent confirmation of the founding metric**. Same distinction as s*/χ² under pin 100
- [ ] **⚖ OWNER PIN 106(c) — THE TRANSFER-READINGS SECTION STATES THAT THE INSTRUMENT COMPOSITION IS INCOMPLETE at all four diverse tiles.** It belongs where a reader meets the NUMBERS, not only in a `recorded_absences` row they may never open: `groundtrack` — the reference-free family's founding member, the **0.410→0.331 lineage** — is ABSENT at every diverse tile because the only orbit-geometry provider is CHALLENGE-BOX scoped (`build_geometry_artifact` fixes `_LON_LO`/`_LON_HI` and φ0 = 38.1). Policy (b) pins the composition, so **the pack must not present four readings as though that composition were satisfied**. State it as pin 106(e) states it: **a REAL WEAKENING of the deliverable**, accepted because absence honestly recorded beats geometry that does not belong to the tile — *not* because the gap is small. **Name the design conflict (106b)**: "GroundTrack per tile×era" and "zero new surfaces" are incompatible while the provider is challenge-box scoped — a design conflict Stage 2 INHERITS, never an execution gap it should rediscover as a bug
...
- [ ] **⚖ OWNER PIN 124(b) — THE VERIFY COMMAND IS THE GATE SUITE IN PIN 83'S ORDER**, not a bare pytest: `phase14_gate_suite.py run` (format → mypy → stamp → suite → re-verify) then `verify`. A bare pytest reproduces the exact defect pin 83 closed — gate evidence from a pre-format tree, structurally and every time. T5e ran this for real (1543 passed, tree unchanged across the run, `verify` PASS against a stamp recording a COMPLETED suite), so the command is established rather than hypothetical
...
```json:metadata
{"userGate": true, "tags": ["user-gate"], "files": ["docs/superpowers/2026-XX-phase14-gate1-pack.md"], "verifyCommand": "pixi run python scripts/phase14_gate_suite.py run && pixi run python scripts/phase14_gate_suite.py verify", "acceptanceCriteria": ["pack posted with the nine sections", "full sweep green on final tree, skips named", "dual reviews cited", "STOP posted"], "requireEvidenceTokens": [["anchor", "five-gate", "identity"], ["seam", "verdict"], ["transfer", "tiles"]]}
```
...
- [ ] **⚖ OWNER PIN 106(d) — the contract carries PER-TILE ORBIT GEOMETRY as NAMED STAGE-2 WORK**, with its founding-metric context attached: this is the **0.410→0.331 lineage**, the spec's own standing reference-free instrument, **absent at exactly the tiles the stage was built to measure**, because the Phase-11 provider is challenge-box scoped. Cheap to record now, expensive to reconstruct later. Carried forward as work, NOT as a Stage-1 defect — Stage 1's wiring is correct and its absences are honestly recorded (fork F)
...
- **The seal is read-only context:** `seal_run check` must PASS unchanged at T9. Any epoch/gauge/config drift discovered mid-stage is a STOP-for-owner, not a reseal. **ONE AUTHORIZED EXCEPTION, NOT YET SPENT:** the owner ruling of 2026-07-27 (pins 32, 34, 36-38) amends the SEALED seam rubric, so T13 is authorized to supersede seal `a17ea419…b725c5d2` through the new `phase14_seal_run.py supersede` path. T13's first two attempts were sealed locally and WITHDRAWN before commit (pin 39's reviews overturned the derived factor; see `docs/superpowers/2026-07-27-t13-adversarial-reviews.md`), so **the sealed record is still at v1 and the exception remains available for exactly one use.** Beyond it, unchanged: a reseal needs its own owner decision.
- **Tasks 13–16 come from the owner ruling of 2026-07-27**, not from this plan's original design pass. Their acceptance criteria live in `2026-07-23-phase14-stage1-spatial-2017.md.tasks.json` (ids 13–16) and their reasoning in `docs/superpowers/2026-07-27-owner-ruling-crn-sigma-rule0.md` — read the ruling, not a summary. Order: **T13 (IN PROGRESS — withdrawn after review, blocked on three owner items) → T14 → T15 → T16**, with the owner's STOP at the end of T14 and T12 absorbing the amended-rubric coverage re-walk (pin 35). T5 is additionally `blockedBy [14]` on top of its standing Tier-2 WAIT.
